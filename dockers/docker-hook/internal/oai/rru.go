...
func startRru(OaiObj Oai) error {
	///////////////
	...
	// Get du ip
	OaiObj.Logger.Print("rccDomain=", rccDomain)
	rccIP, err := util.GetIPFromDomain(OaiObj.Logger, rccDomain)
	if err != nil {
		OaiObj.Logger.Print(err)
		rccIP = "192.168.12.45"
	}
	...
	util.RunCmd(OaiObj.Logger, "sed", "-i", sedCommand, enbConf)
	// remote_address
	sedCommand = "s:remote_address.*;:      remote_address 			      = \"" + rccIP + "\";:g"
	util.RunCmd(OaiObj.Logger, "sed", "-i", sedCommand, enbConf)
	// local_portc
	//START HERE
	sedCommand = "s:local_portc.*;:      local_portc 			      = " + c.RruPortc.Default + ";:g"
	util.RunCmd(OaiObj.Logger, "sed", "-i", sedCommand, enbConf)
	// remote_portc
	sedCommand = "s:remote_portc.*;:      remote_portc 			      = " + c.RccPortc.Default + ";:g"
	util.RunCmd(OaiObj.Logger, "sed", "-i", sedCommand, enbConf)
	// local_portd
	sedCommand = "s:local_portd.*;:      local_portd 			      = " + c.RruPortd.Default + ";:g"
	util.RunCmd(OaiObj.Logger, "sed", "-i", sedCommand, enbConf)
	// remote_portd
	sedCommand = "s:remote_portd.*;:      remote_portd 			      = " + c.RccPortd.Default + ";:g"
	util.RunCmd(OaiObj.Logger, "sed", "-i", sedCommand, enbConf)
	// tr_preference
...
func stopRan(OaiObj Oai) error {
	///////////////
	OaiObj.Logger.Print("Stop RAN Service")
	for {
		retStatus := util.RunCmd(OaiObj.Logger, "/snap/bin/oai-ran.enb-stop")
		if len(retStatus.Stderr) == 0 {
			break
		}
		OaiObj.Logger.Print("Stop RAN failed, try again later")
		time.Sleep(1 * time.Second)
	}
	return nil
}
