...
class ConfigManager(object):
    def __init__(self, conf_short, conf_global_default, config_short, entities_cicd, snap_channel_cicd, snap_devmode_cicd, docker_reg_cicd, docker_tag_v1_cicd, docker_tag_v2_cicd):
        ...
        if config_short:
            # open short config file and configure global (long) config file
    ...
    def config_cicd(self, entity_cicd, snap_channel_cicd, snap_devmode_cicd, docker_reg_cicd, docker_tag_v1_cicd, docker_tag_v2_cicd):
        ...
        if entity_cicd == "all":
            ...
            config_cicd_oaiHss(self)
            config_cicd_oaiSpgw(self)
            config_cicd_oaiSpgwc(self)
            config_cicd_oaiSpgwu(self)
            config_cicd_oaiMme(self)
            config_cicd_oaiCn(self)
            config_cicd_flexran(self)
        elif entity_cicd == "oai-ran":
            config_cicd_oairan(self)
        elif entity_cicd == "oai-hss":
            config_cicd_oaiHss(self)
        elif entity_cicd == "oai-spgw":
            config_cicd_oaiSpgw(self)
        elif entity_cicd == "oai-spgwc":
            config_cicd_oaiSpgwc(self)
        elif entity_cicd == "oai-spgwu":
            config_cicd_oaiSpgwu(self)
        elif entity_cicd == "oai-mme":
            config_cicd_oaiMme(self)
        elif entity_cicd == "oai-cn":
            config_cicd_oaiCn(self)
        elif entity_cicd == "flexran":
            config_cicd_flexran(self)
        else:
            logger.error("The entity {} is not supported yet, exit ...".format(entity_cicd))
            exit(0)
    ...
    def config_docker_build(self):
        ...
        try:
            ...
            logger.debug("configuration for docker-build config file is successfully written in {}".format(conf_file_out_docker_build))
        except Exception as ex:
            message = "Error while trying to open the file: {}".format(ex) 
            logger.error(message)
            exit(0)
    ...
    def config_lte_all_in_one_with_flexran(self, version):
        ...
        # change mme config of oaiEnb 
        conf_crs_lte_all_in_one_with_flexran_data = copy.deepcopy(self.config_global_data)
        conf_crs_lte_all_in_one_with_flexran_data["spec"]["oaiEnb"][0]["mmeService"]["snapVersion"] = version
        conf_crs_lte_all_in_one_with_flexran_data["spec"]["oaiEnb"][0]["mmeService"]["name"] = \
                conf_crs_lte_all_in_one_with_flexran_data["spec"]["oaiCn"][version][0]["k8sServiceName"]
        conf_crs_lte_all_in_one_with_flexran_data["spec"]["oaiEnb"][0]["flexRAN"] = True
        # delete "oaiCn" 
        try:
            del conf_crs_lte_all_in_one_with_flexran_data["spec"]["oaiCn"][alter_version]
        except:
            logger.debug("the key {} does not exist in {}, skipping".format(alter_version, conf_crs_lte_all_in_one_with_flexran_data["spec"]["oaiCn"]))
        
        # delete "llmec", "oaiHss", "oaiMme", "oaiSpgw", "oaiSpgwc", "oaiSpgwu"        
        oaispgwu_param = ["llmec", "oaiHss", "oaiMme", "oaiSpgw", "oaiSpgwc", "oaiSpgwu"]
        for key in oaispgwu_param:
            try:
                del conf_crs_lte_all_in_one_with_flexran_data["spec"][key]
            except:
                logger.debug("the key {} does not exist in {}, skipping".format(key, conf_crs_lte_all_in_one_with_flexran_data["spec"]))

        # getting the right database
        for item in range(len(conf_crs_lte_all_in_one_with_flexran_data["spec"]["database"])):
            if conf_crs_lte_all_in_one_with_flexran_data["spec"]["database"][item]["databaseType"] == alter_database_type:
                del conf_crs_lte_all_in_one_with_flexran_data["spec"]["database"][item]
                break
        try:
            with open(conf_file_out_crs, 'w') as outfile:
                ruamel.yaml.dump(conf_crs_lte_all_in_one_with_flexran_data, outfile, Dumper=ruamel.yaml.RoundTripDumper)
            self.list_configured_files_crs.append(conf_file_out_crs)
            logger.debug("configuration of crs for lte_all_in_one {} is successfully written in {}".format(version, conf_file_out_crs))
        except Exception as ex:
            message = "Error while trying to open the file: {}".format(ex)
        ...
        ## Docker config
        # getting the config from global 
        conf_docker_lte_all_in_one_with_flexran_data = copy.deepcopy(conf_crs_lte_all_in_one_with_flexran_data['spec'])
        self.open_docker_compose(docker_compose_file)
    ...
    def config_lte(self, version):
        ...
        try:
            ...
            logger.error(message)
            exit(0)
    # config docker compose and crs of kube5g-operator for lte-with-flexran
    def config_lte_with_flexran(self, version):
        logger.debug("configuring crs of lte with flexran for the version {}".format(version))
        alter_version = "v1" if version == "v2" else "v2"
        alter_database_type = "cassandra" if version == "v1" else "mysql"
        database_type = "mysql" if version == "v1" else "cassandra"
        conf_file_out_crs = "{}/cr-{}/lte-with-flexran/mosaic5g_v1alpha1_cr_{}_lte_flexran.yaml".format(self.common_dir_crs, version, version)
        
        conf_file_out_docker = "{}/oai-{}/lte-with-flexran/conf.yaml".format(self.common_dir_docker, version)
        docker_compose_file = "{}/oai-{}/lte-with-flexran/docker-compose.yaml".format(self.common_dir_docker, version)
        docker_compose_file_output = "{}/oai-{}/lte-with-flexran/docker-compose.yaml".format(self.common_dir_docker, version)
        """                    crs v1/v2 lte-with-flexran                    """
        # change mme config of oaiEnb
        conf_crs_lte_data = copy.deepcopy(self.config_global_data)
        conf_crs_lte_data["spec"]["oaiEnb"][0]["mmeService"]["snapVersion"] = version
        conf_crs_lte_data["spec"]["oaiEnb"][0]["mmeService"]["name"] = \
                conf_crs_lte_data["spec"]["oaiMme"][version][0]["k8sServiceName"]
        conf_crs_lte_data["spec"]["oaiEnb"][0]["flexRAN"] = True
        # delete, llmec, oaiCn
        entity_param = ["llmec", "oaiCn"]
        for key in entity_param:
            try:
                del conf_crs_lte_data["spec"][key]
            except:
        ...
        self.docker_compose_data["services"]["oaihss"]["image"] = conf_docker_lte_data["oaiHss"][version][0]["oaiHssImage"]
        self.docker_compose_data["services"]["oaimme"]["image"] = conf_docker_lte_data["oaiMme"][version][0]["oaiMmeImage"]
        self.docker_compose_data["services"]["flexran"]["image"] = conf_docker_lte_data["flexran"][0]["flexranImage"]
        # getting names of docker and other values for the concerned entities
        ## oaiEnb
        conf_docker_lte_data["oaiEnb"][0]["mmeService"]["snapVersion"] = version
        conf_docker_lte_data["oaiEnb"][0]["mmeService"]["name"] = self.docker_compose_data["services"]["oaimme"]["container_name"]
        conf_docker_lte_data["oaiEnb"][0]["flexRAN"] = True
        conf_docker_lte_data["oaiEnb"][0]["flexRANServiceName"] = self.docker_compose_data["services"]["flexran"]["container_name"]
        ## oaiHss
        conf_docker_lte_data["oaiHss"][version][0]["databaseServiceName"] = self.docker_compose_data["services"][database_type]["container_name"]
        conf_docker_lte_data["oaiHss"][version][0]["hssServiceName"] = self.docker_compose_data["services"]["oaihss"]["container_name"]
        conf_docker_lte_data["oaiHss"][version][0]["mmeServiceName"] = self.docker_compose_data["services"]["oaimme"]["container_name"]
        ...
        for key in flexran_param_total:
            try:
                del conf_docker_lte_data["flexran"][0][key]
            except:
                logger.debug("the key {} does not exist in {}, skipping".format(key, conf_docker_lte_data["flexran"][0]))
        # oaiEnb
        oaienb_param_total = oaienb_param + k8s_param
        for key in oaienb_param_total:
            try:
